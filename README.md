...
Действие процессора находится в ACTIONS рядом с MAC; reset остаётся в SERVICE.
Строка `Processor ID` показывает текущий OTP: `(not in EEPROM)` при отсутствии
сохранённой привязки, `(in EEPROM)` при совпадении. При несовпадении показываются
текущий и сохранённый ID с `PROCESSOR MISMATCH`; при ошибке чтения — `OTP UNAVAILABLE`.
...
через `yes`, сохраняя serial/MAC. Для пустой EEPROM показывает всю текущую
конфигурацию и OTP и предлагает записать их целиком в v3 с отдельным `yes`,
backup и read-back. На существующей v2 объясняет необходимость reset или
