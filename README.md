...
- `--otp PATH` — путь к RK3308 NVMEM для processor, mac, eeprom write и TUI;
...
новые значения → `Write EEPROM` (текущий OTP включён в preview).
...
`processor write/bind/rebind` также доступны с явным подтверждением. На существующей v2 объясняет необходимость reset или
явной миграции. Для CLI `processor write --config instance.json` задаёт поля
